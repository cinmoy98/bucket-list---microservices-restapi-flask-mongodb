...
import shortuuid
from app.BucketModel.notes import Note
...
bapp.config["MONGO_URI"] = "mongodb://localhost:27017/bucket_list"
# bapp.config['SECRET_KEY'] = "Thisisreallysecret"
# bapp.config['JWT_SECRET_KEY'] = "DoNotExpose"
# bapp.config['JWT_TOKEN_LOCATION'] = ['headers', 'query_string']
# bapp.config['JWT_HEADER_NAME'] = ["Authorization"]
# bapp.config['JWT_HEADER_TYPE'] = ["Bearer"]
...
@bapp.route('/api/bucket/newNote',methods=['POST'])
@jwt_required()
def create_note():
...
@bapp.route('/api/bucket/allNotes',methods=['GET'])
@jwt_required()
def get_notes():
	try:
...
@bapp.route('/api/bucket/notesByQuery', methods=['GET'])
@jwt_required()
...
@bapp.route('/api/bucket/note/<string:note_id>',methods=['GET'])
...
@bapp.route('/api/bucket/distictCat', methods=['GET'])
@jwt_required()
def get_categories():
	try:
		uid = get_jwt_identity()
...
# @bapp.route('/get_category/<catg>',methods=['GET'])
# @jwt_required()
...
# @bapp.route('/get_country/<string:country>', methods=['GET'])
...
# @bapp.route('/get_city/<string:city>', methods=['GET'])
# def get_city(city):
...
@bapp.route('/api/bucket/cities/<string:country>', methods=['GET'])
@jwt_required()
...
@bapp.route('/api/bucket/distinctCountries',methods=['GET'])
@jwt_required()
def get_countries():
	try:
		uid = get_jwt_identity()
...
@bapp.route('/delete_note/<note_id>',methods=['POST'])
def delete_note(note_id):
	Note.delete_note_by_id(note_id, mongo)
	return "Deleted !"


@bapp.route('/add_yt_link/<note_id>', methods=['POST'])
...
@bapp.route('/add_fb_link/<note_id>', methods=['POST'])
...
@bapp.route('/add_blog_link/<note_id>', methods=['POST'])
...
@bapp.route('/add_insta_link/<note_id>', methods=['POST'])
